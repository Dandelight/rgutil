"""
Extension of pysnooper, introduced at <https://github.com/zasdfgbnm/TorchSnooper>
"""
...
FLOATING_POINTS = set()
for i in ['float', 'double', 'half', 'bfloat16', 'complex128', 'complex32', 'complex64']:
...
class TensorFormat:
    ...
    def __init__(self, property_name=False, properties=('shape', 'dtype', 'device', 'requires_grad', 'has_nan', 'has_inf', 'memory_format')):
    ...
    def repr_memory_format(self, tensor):
        ctg = tensor.is_contiguous(memory_format=torch.contiguous_format)
        cl = tensor.is_contiguous(memory_format=torch.channels_last)
        cl3d = tensor.is_contiguous(memory_format=torch.channels_last_3d)
        if ctg:
            mf = 'contiguous'
        elif cl:
            mf = 'channels_last'
