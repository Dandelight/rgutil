...
class RandomAffine(object):
    ...
    def __call__(self, img):
        ...
        """
        ret = self.get_params(
            self.degrees, self.translate, self.scale, self.shear, img.shape
        )
...
class RandomPerspective(object):
    ...
    def __init__(
        ...
        self.anglex = (
            (-anglex, anglex) if isinstance(anglex, numbers.Number) else anglex
        )
        self.angley = (
            (-angley, angley) if isinstance(angley, numbers.Number) else angley
        )
        self.anglez = (
            (-anglez, anglez) if isinstance(anglez, numbers.Number) else anglez
        )
    ...
    def get_params(
        ...
        scale = (
            random.uniform(1 / scale_ranges[0], scale_ranges[0]),
...
class RandomAffine6(object):
    ...
    def __init__(
        ...
        if isinstance(anglez, numbers.Number):
            ...
            assert (
                isinstance(anglez, (tuple, list)) and len(anglez) == 2
    ...
    def __call__(self, img):
        ...
        """
        ret = self.get_params(
            img.shape, self.anglez, self.shear, self.translate, self.scale
        )
...
class RandomGaussianNoise(object):
    ...
    def __init__(self, p=0.5, mean=0, std=0.1):
        assert (
            isinstance(mean, numbers.Number) and mean >= 0
        ), "mean should be a positive value"
        assert (
            isinstance(std, numbers.Number) and std >= 0
        ), "std should be a positive value"
...
class RandomSPNoise(object):
    ...
    def __init__(self, p=0.5, prob=0.1):
        assert isinstance(p, numbers.Number) and p >= 0, "p should be a positive value"
        assert (
            isinstance(prob, numbers.Number) and prob >= 0
        ), "p should be a positive value"
        self.p = p
        self.prob = prob
